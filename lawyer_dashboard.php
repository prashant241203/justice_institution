...
<td><span class="badge bg-<?= $badge ?>"><?= $case['status'] ?></span></td>
<td><?= $case['hearing_count'] ?></td>

<td class="d-flex gap-1">
  <a href="view_case.php?case_id=<?= urlencode($case['case_id']) ?>"
     class="btn btn-sm btn-primary">View Case</a>

  <a href="view_hearing.php?case_id=<?= urlencode($case['case_id']) ?>#hearings"
     class="btn btn-sm btn-outline-secondary">Hearings</a>

  <?php if (can('upload_document')): ?>
    <a href="upload_document.php?case_id=<?= urlencode($case['case_id']) ?>&from=dashboard"
       class="btn btn-sm btn-outline-success">Upload Docs</a>
  <?php endif; ?>
</td>
</tr>
<?php endwhile; ?>
