...
<html lang="en">
  ...
  <body>
    <div class="container mt-5">
      <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      </div> <?php if(isset($error)) { echo "
					<div class='alert alert-danger'>$error</div>"; } ?> <form action="" method="POST" class="card p-4 shadow-sm">
        <div class="mb-3">
        ...
        <div class="mb-3">
          ...
          <select class="form-select" name="lawyer_id" id="lawyer_id" required>
            <option value="">Select Lawyer</option> <?php while($lawyer = mysqli_fetch_assoc($lawyerResult)) { ?> <option value="
											<?= $lawyer['user_id'] ?>"> <?= htmlspecialchars($lawyer['name']) ?> </option> <?php } ?>
          </select>
        </div>
